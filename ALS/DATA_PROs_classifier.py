...
def RF_classifier(factor0, labels, train_idx, val_idx, test_idx, importance_threshold=0.05, final_phenotype_indices=None):
    ...
    # Print metrics
    print(f"AUC for yr2 outcome: {auc_1:.3f}")
    print(f"Precision for yr2 outcome: {precision_1:.3f}")
    print(f"Recall for yr2 outcome: {recall_1:.3f}")
    print(f"F1 score for yr2 outcome: {f1_1:.3f}")

    print(f"AUC for yr3 outcome: {auc_2:.3f}")
    print(f"Precision for yr3 outcome: {precision_2:.3f}")
    print(f"Recall for yr3 outcome: {recall_2:.3f}")
    print(f"F1 score for yr3 outcome: {f1_2:.3f}")
    ...
    features = [f"Phenotype {i + 1}" for i in range(x_matrix.shape[1])] # these are 1-indexed
