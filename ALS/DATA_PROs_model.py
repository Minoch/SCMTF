...
class JointDecompositionModel(nn.Module):
    ...
    def forward(self, samples):
        ...
        pred = self.classifier(self.factor0[samples])
        return pred
...
def logistic_regression(Wtrain, y_train, Wtest, y_test):
    ...
    pred_prob_train = [clf.predict_proba(Wtrain)[:, 1] for clf in classifier.estimators_]
    pred_prob_train = np.column_stack(pred_prob_train)

    log_loss_1 = log_loss(y_train[:, 0], pred_prob_train[:, 0], eps=1e-15)
    log_loss_2 = log_loss(y_train[:, 1], pred_prob_train[:, 1], eps=1e-15)

    pred_prob_test = [clf.predict_proba(Wtest)[:, 1] for clf in classifier.estimators_]
    pred_prob_test = np.column_stack(pred_prob_test)

    prauc_1 = roc_auc_score(y_test[:, 0], pred_prob_test[:, 0])
    prauc_2 = roc_auc_score(y_test[:, 1], pred_prob_test[:, 1])

    predictions = classifier.predict(Wtest)

    return prauc_1, prauc_2, log_loss_1, log_loss_2, predictions

########################################## WITH BIAS ##########################################
def run_model_with_bias(tensor, tensor1, Mtensor, 
              rng, device, 
    ...
    # A
    optimizer1_f0 = torch.optim.Adam([model.weights, model.factor0, patient_uniform.slice_values, feature_uniform.slice_values], lr=lr) # already has momentum
    # B
    optimizer1_f1 = torch.optim.Adam([model.weights, model.factor1, patient_uniform.slice_values, feature_uniform.slice_values], lr=lr) # already has momentum
    # C
    optimizer1_f2 = torch.optim.Adam([model.weights, model.factor2, patient_uniform.slice_values, feature_uniform.slice_values], lr=lr) # already has momentum
    # D
    optimizer1_mf1 = torch.optim.Adam([model.weights, model.Mfactor1, patient_uniform.slice_values, feature_uniform.slice_values], lr=lr) # already has momentum

    # Add learning rate schedulers
    # A
    scheduler1_f0 = optim.lr_scheduler.StepLR(optimizer1_f0, step_size=1000, gamma=0.8)
    # B
    scheduler1_f1 = optim.lr_scheduler.StepLR(optimizer1_f1, step_size=1000, gamma=0.8)
    # C
    scheduler1_f2 = optim.lr_scheduler.StepLR(optimizer1_f2, step_size=1000, gamma=0.8)
    # D
    scheduler1_mf1 = optim.lr_scheduler.StepLR(optimizer1_mf1, step_size=1000, gamma=0.8)
    ...
    for i in range(1, n_epoch):
        ...
        ######## Optimize factor 0 ########
        model.factor0.requires_grad = True
        model.factor1.requires_grad = False
        model.factor2.requires_grad = False
        model.Mfactor1.requires_grad = False
        optimizer1_f0.zero_grad()
        ...
        loss_tot = (1 - Lambda) * loss + Lambda * loss_cl # carry over the classifier loss from the previous loop to this common matrix
        ...
        ######## Optimize factor 1 ########
        model.factor0.requires_grad = False
        model.factor1.requires_grad = True
        model.factor2.requires_grad = False
        model.Mfactor1.requires_grad = False
        optimizer1_f1.zero_grad()
        # Reconstruct the tensor from the decomposed form
        rec = relu(cp_to_tensor((model.weights, [model.factor0,model.factor1,model.factor2]))) + patient_bias + feature_bias
        Mrec = relu(cp_to_tensor((model.weights, [model.factor0,model.Mfactor1]))) 
        # Calculate reconstruction loss
        ...
        model.factor0.requires_grad = False
        model.factor1.requires_grad = False
        model.factor2.requires_grad = True
        model.Mfactor1.requires_grad = False
        optimizer1_f2.zero_grad()
        # Reconstruct the tensor from the decomposed form
        rec = relu(cp_to_tensor((model.weights, [model.factor0,model.factor1,model.factor2]))) + patient_bias + feature_bias
        Mrec = relu(cp_to_tensor((model.weights, [model.factor0,model.Mfactor1]))) 
        # Calculate reconstruction loss
        ...
        model.factor1.requires_grad = False
        model.factor2.requires_grad = False
        model.Mfactor1.requires_grad = True
        optimizer1_mf1.zero_grad()
        # Reconstruct the tensor from the decomposed form
        rec = relu(cp_to_tensor((model.weights, [model.factor0,model.factor1,model.factor2]))) + patient_bias + feature_bias
        Mrec = relu(cp_to_tensor((model.weights, [model.factor0,model.Mfactor1]))) 
        # Calculate reconstruction loss
        ...
        _, _, log_loss_2, log_loss_3, _ = logistic_regression(model.factor0[train_idx].cpu().detach().numpy(), labels[train_idx].cpu().detach().numpy(), 
                                                                                model.factor0[val_idx].cpu().detach().numpy(), labels[val_idx].cpu().detach().numpy())
        loss_cl = log_loss_2 + log_loss_3
    ...
    print(labels[train_idx].dtype)
    _, _, _, _, pred = logistic_regression(model.factor0[train_idx].cpu().detach().numpy(), labels[train_idx].cpu().detach().numpy(), 
                                                                                model.factor0[final_idx].cpu().detach().numpy(), labels[final_idx].cpu().detach().numpy())
    ...
    outcome2_auc = roc_auc_score(labels[final_idx][:,0].cpu().detach().numpy(), pred[:,0])
    outcome3_auc = roc_auc_score(labels[final_idx][:,1].cpu().detach().numpy(), pred[:,1])

    # Additional metrics
    outcome2_pred_labels = (pred[:,0] > 0.5)
    outcome2_true_labels = labels[final_idx][:,0]

    # Outcome 3
    outcome3_pred_labels = (pred[:,1] > 0.5)
    outcome3_true_labels = labels[final_idx][:,1]
...
########################################## WITHOUT BIAS ##########################################
def run_model_without_bias(tensor, tensor1, Mtensor, 
              rng, device, 
              train_idx, test_idx, val_idx, labels, 
              rank, n_epoch, Lambda, lr, 
              penalty_l1, penalty_l2, theta, grid=False):
    ...
    # A
    optimizer1_f0 = torch.optim.Adam([model.weights, model.factor0], lr=lr) # already has momentum
    # B
    optimizer1_f1 = torch.optim.Adam([model.weights, model.factor1], lr=lr) # already has momentum
    # C
    optimizer1_f2 = torch.optim.Adam([model.weights, model.factor2], lr=lr) # already has momentum
    # D
    optimizer1_mf1 = torch.optim.Adam([model.weights, model.Mfactor1], lr=lr) # already has momentum
    ...
    for i in range(1, n_epoch):
        ...
        optimizer1_f0.zero_grad()
        # Reconstruct the tensor from the decomposed form
        rec = relu(cp_to_tensor((model.weights, [model.factor0,model.factor1,model.factor2])))
        Mrec = relu(cp_to_tensor((model.weights, [model.factor0,model.Mfactor1]))) 
        # Calculate reconstruction loss
        Tloss = tl.norm(mask12 * (rec - tensor), 2)
        Mloss = tl.norm(Mrec - Mtensor, 2)
    ...
    ### Evaluate model on test set
    factors = [model.factor0, model.factor1, model.factor2]
    rec = relu(cp_to_tensor((model.weights, factors)))
    tl.norm(((~mask12)*mask1)*(rec - tensor1), 2)/tl.norm(((~mask12)*mask1)*tensor1, 2)
    ...
    result_dict['temporal_x_rank'] = model.factor2
    result_dict['weights'] = model.weights
    result_dict['patient bias'] = None
    result_dict['feature bias'] = None
